...
public class Google extends DriverClass {
	...
	String[] xlSheet = {"Games","Movies"};

	public void imageTest() throws IOException {
		test = extent.createTest("This is testing Images");
		for(int j=0;j<2;j++)
		{
		int n = ExcelUtil.rowCount(xlFile, xlSheet[j]);
		for (int i = 1; i <= n; i++) {
			driver.findElement(GooObj.objImg).click();
			String value = ExcelUtil.getCellData(xlFile, xlSheet[j], i, 1);
			...
			if (url.toLowerCase().contains("img")) {
				...
				ExcelUtil.setCellData(xlFile, xlSheet[j], i, 2, "PASSED");
				if(value.toLowerCase().contains("prince"))
				{
					ExcelUtil.setCellData(xlFile, xlSheet[j], i, 2, "FAILED");
				}
			} else {
